...
def main(args):
    ...
    TRAIN_DATASET = PrimitiveDataset(root=root, npoints=args.npoint, split='train', pred_start_only=False)
    trainDataLoader = torch.utils.data.DataLoader(TRAIN_DATASET, batch_size=args.batch_size, shuffle=True, num_workers=10, drop_last=True)
    TEST_DATASET = PrimitiveDataset(root=root, npoints=args.npoint, split='test', pred_start_only=False)
    testDataLoader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=args.batch_size, shuffle=False, num_workers=10)
    log_string("The number of training data is: %d" % len(TRAIN_DATASET))
    log_string("The number of test data is: %d" % len(TEST_DATASET))

    num_outputs = 14 # (3 offset + 4 quaternion) * 2 for start/end
    inp_dim = 7 # (xyz, rgb, keypoint mask)
    num_classes = 4 # (nothing, start waypoint, end waypoint, both)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        '''learning one epoch'''
        for i, (points, cls_target, start_waypt, end_waypt, start_rot, end_rot) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            optimizer.zero_grad()
            ...
            end_waypt_batched = torch.zeros((args.npoint, args.batch_size, 3)).cuda()
            end_waypt_batched[:,:,:] = end_waypt
            start_waypt_batched = start_waypt_batched.transpose(0,1)
            end_waypt_batched = end_waypt_batched.transpose(0,1)

            start_rot_batched = torch.zeros((args.npoint, args.batch_size, 4)).cuda()
            start_rot_batched[:,:,:] = start_rot
            end_rot_batched = torch.zeros((args.npoint, args.batch_size, 4)).cuda()
            end_rot_batched[:,:,:] = end_rot
            start_rot_batched = start_rot_batched.transpose(0,1)
            end_rot_batched = end_rot_batched.transpose(0,1)

            points = torch.Tensor(points)
            points, cls_target, start_waypt, end_waypt, start_rot, end_rot = points.float().cuda(), \
                                                                             cls_target.long().cuda(), \
                                                                             start_waypt.float().cuda(), \
                                                                             end_waypt.float().cuda(), \
            ...
            #print('HERE', sum(cls_target==0), sum(cls_target==1), sum(cls_target==2), sum(cls_target==3))

            pred_choice = cls_pred.data.max(1)[1]

            start_idxs = (pred_choice == 1).view(args.batch_size, args.npoint)
            end_idxs = (pred_choice == 2).view(args.batch_size, args.npoint)
            both_idxs = (pred_choice == 3).view(args.batch_size, args.npoint)
            start_idxs = torch.logical_or(start_idxs, both_idxs)
            end_idxs = torch.logical_or(end_idxs, both_idxs)

            if not torch.any(end_idxs):
                end_idxs = start_idxs
            ...
            offsets_end = points_xyz[end_idxs] - end_waypt_batched[end_idxs]
            offsets_end_pred = offsets_pred[:,:,3:][end_idxs]

            #print('len start idxs', offsets_start.shape)
            #print('len end idxs', offsets_end.shape)

            rots_start_pred = rots_pred[:,:,:4][start_idxs]
            rots_start = start_rot_batched[start_idxs]
            rots_end_pred = rots_pred[:,:,4:][end_idxs]
            rots_end = end_rot_batched[end_idxs]

            norm_start = 1/torch.sqrt(torch.sum(rots_start_pred*rots_start_pred, axis=1))
            norm_end = 1/torch.sqrt(torch.sum(rots_end_pred*rots_end_pred, axis=1))
            rots_start_pred_norm = (rots_start_pred.permute(1,0) * norm_start).permute(1,0)
            rots_end_pred_norm = (rots_end_pred.permute(1,0) * norm_end).permute(1,0)
            ...
            end_offset_loss = offset_criterion(offsets_end_pred, offsets_end)

            start_rot_loss = rot_criterion(rots_start_pred_norm, rots_start)
            end_rot_loss = rot_criterion(rots_end_pred_norm, rots_end)

            mean_start_offset_loss.append(start_offset_loss.item())
            mean_end_offset_loss.append(end_offset_loss.item())
            mean_start_rot_loss.append(start_rot_loss.item())
            mean_end_rot_loss.append(end_rot_loss.item())

            #alpha = 0.2
            #beta1 = 0.15
            #beta2 = 0.15
            #gamma1 = (1-alpha-beta1-beta2)*0.5
            #gamma2 = (1-alpha-beta1-beta2)*0.5
            ...
            gamma1 = (1-alpha-beta1-beta2)*0.5
            gamma2 = (1-alpha-beta1-beta2)*0.5
            loss = alpha*cls_loss + gamma1*start_offset_loss + gamma2*end_offset_loss + beta1*start_rot_loss + beta2*end_rot_loss
        ...
        log_string('Train end offset loss is: %.5f' % np.mean(mean_end_offset_loss))
        log_string('Train start rot loss is: %.5f' % np.mean(mean_start_rot_loss))
        log_string('Train end rot loss is: %.5f' % np.mean(mean_end_rot_loss))
        ...
        with torch.no_grad():
            ...
            for batch_id, (points, cls_target, start_waypt, end_waypt, start_rot, end_rot) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
                ...
                start_waypt_batched[:,:,:] = start_waypt
                end_waypt_batched = torch.zeros((args.npoint, cur_batch_size, 3)).cuda()
                end_waypt_batched[:,:,:] = end_waypt
                start_waypt_batched = start_waypt_batched.transpose(0,1)
                end_waypt_batched = end_waypt_batched.transpose(0,1)
                ...
                start_rot_batched[:,:,:] = start_rot
                end_rot_batched = torch.zeros((args.npoint, cur_batch_size, 4)).cuda()
                end_rot_batched[:,:,:] = end_rot
                start_rot_batched = start_rot_batched.transpose(0,1)
                end_rot_batched = end_rot_batched.transpose(0,1)
                ...
                end_idxs = (pred_choice == 2).view(cur_batch_size, args.npoint)
                both_idxs = (pred_choice == 3).view(cur_batch_size, args.npoint)
                start_idxs = torch.logical_or(start_idxs, both_idxs)
                end_idxs = torch.logical_or(end_idxs, both_idxs)
                ...
                #start_rot_loss = offset_criterion(rots_start_pred_norm, rots_start)
                #end_rot_loss = offset_criterion(rots_end_pred_norm, rots_end)
                start_rot_loss = rot_criterion(rots_start_pred_norm, rots_start)
                end_rot_loss = rot_criterion(rots_end_pred_norm, rots_end)

                if not np.isnan(start_offset_loss.item()):
                    mean_start_offset_loss.append(start_offset_loss.item())
                if not np.isnan(end_offset_loss.item()):
                    mean_end_offset_loss.append(end_offset_loss.item())
                if not np.isnan(start_rot_loss.item()):
                    mean_start_rot_loss.append(start_rot_loss.item())
                if not np.isnan(end_rot_loss.item()):
                    mean_end_rot_loss.append(end_rot_loss.item())
        ...
        log_string('Test end offset loss is: %.5f' % np.mean(mean_end_offset_loss))
        log_string('Test start rot loss is: %.5f' % np.mean(mean_start_rot_loss))
        log_string('Test end rot loss is: %.5f' % np.mean(mean_end_rot_loss))

        #if test_instance_acc > best_test_acc:
        if np.mean(mean_end_rot_loss) < best_offset:
            ...
            best_offset = np.mean(mean_end_rot_loss)
